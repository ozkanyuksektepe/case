            ...
            <div class="card-header">
                ...
                <div class="col-12 col-md">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="card-header-title">Blog Kategorileri ({{ $category->count() }})</h4>
                    </div>
                </div>
                    <div class="col-auto">
                        <!-- Filter -->
                        <div class="row align-items-sm-center">
                            ...
                            <div class="mr-3 ml-4">
                                <a title="Yeni Ekle" href="{{ route('panel.blog-category.create') }}" class="btn btn-sm btn-success btn-spinner-show"><i class="tio-add"></i> Yeni Ekle</a>
                            </div>
                        </div>
                        <!-- End Filter -->
                    </div>
                ...
                <table id="datatable" class="table table-hover table-thead-bordered table-nowrap table-align-middle card-table" data-hs-datatables-options='{
                    ...
                    <tbody>
                        @foreach($category as $cat)
                            @php $imgName = $cat->name ? Str::slug($cat->name) : 'blog-category-img'; @endphp
                            <tr>
                                <td class="table-column-pr-0">#{{ $cat->id }}</td>
                                <td>
                                    <span class="avatar avatar-xl avatar-4by3">
                                        <a title="Resim Yükle" href="{{ route('panel.image.show', ['folder' => 'blog-category', 'item_id' => $cat->id, 'name' => $imgName]) }}"><img class="avatar-img" src="{{ getCover($cat->cover) }}" alt="{{ $cat->name }}"></a>
                                    </span>
                                </td>
                                <td>
                                  {{ $cat->name }}
                                ...
                                <td>{{ $cat->rank }}</td>
                                <td><span class="legend-indicator {{ $cat->status ? 'bg-success' : 'bg-secondary' }}"></span>{{ $cat->status ? 'Aktif' : 'Pasif' }} </td>
                                <td>
                                    <a class="btn btn-sm btn-info" href="{{ route('panel.blog-category.edit', $cat->id) }}" data-toggle="tooltip" data-html="true" title="Düzenle"><i class="tio-new-message font-20"></i></a>
                                    <a class="btn btn-sm btn-danger delete-alert" data-url="{{ route('panel.blog-category.destroy', $cat->id) }}" href="javascript:;" data-toggle="tooltip" data-html="true" title="Sil"><i class="tio-remove-from-trash font-20"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
